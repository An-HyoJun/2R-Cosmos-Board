...
void InitSliceMap()
{
	...
	for(sliceAddr=0; sliceAddr<SLICES_PER_SSD ; sliceAddr++)
	{
		...
		virtualSliceMapPtr->virtualSlice[sliceAddr].mh = 0;
		virtualSliceMapPtr->virtualSlice[sliceAddr].ch = 0;
	}
}
...
void InitDieMap()
{
	...
	for(dieNo=0 ; dieNo<USER_DIES ; dieNo++)
	{
		...
		virtualDieMapPtr->die[dieNo].coldBlock = BLOCK_NONE;
		virtualDieMapPtr->die[dieNo].warmBlock = BLOCK_NONE;
...
void InitBlockMap()
{
	...
	for(dieNo=0 ; dieNo<USER_DIES ; dieNo++)
	{
		for(virtualBlockNo=0; virtualBlockNo<USER_BLOCKS_PER_DIE ; virtualBlockNo++)
		{
			...
			virtualBlockMapPtr->block[dieNo][virtualBlockNo].cold = 0;
...
unsigned int AddrTransWrite(unsigned int logicalSliceAddr)
{
	...
	{
		virtualSliceAddr = logicalSliceMapPtr->logicalSlice[logicalSliceAddr].virtualSliceAddr;
		unsigned int dn = Vsa2VdieTranslation(virtualSliceAddr);
		unsigned int bn = Vsa2VblockTranslation(virtualSliceAddr);
		unsigned int tmh = virtualSliceMapPtr->virtualSlice[virtualSliceAddr].mh;

		virtualSliceMapPtr->virtualSlice[virtualSliceAddr].mh = 0;
		virtualSliceMapPtr->virtualSlice[virtualSliceAddr].ch = 0;


		if(virtualBlockMapPtr->block[dn][bn].cold == 1 && tmh < 2){
			tmh++;
		}
		...
		request_write++;
		if(request_write % 100000 == 0){
			preentgcstatus();
		}
		virtualSliceAddr = FindFreeVirtualSlice();

		virtualSliceMapPtr->virtualSlice[virtualSliceAddr].mh = tmh;
		virtualSliceMapPtr->virtualSlice[virtualSliceAddr].ch = 0;
...
unsigned int FindFreeVirtualSlice()
{
	...
	{
		SelectiveGetFromGcVictimList(dieNo, currentBlock);
...
unsigned int FindFreeVirtualSliceForGc(unsigned int copyTargetDieNo, unsigned int victimBlockNo, unsigned int pageNo)
{
	unsigned int coldBlock, virtualSliceAddr, dieNo;

	virtualSliceAddr = Vorg2VsaTranslation(copyTargetDieNo, victimBlockNo, pageNo);

	unsigned tmh = virtualSliceMapPtr->virtualSlice[virtualSliceAddr].mh;
	unsigned tch = virtualSliceMapPtr->virtualSlice[virtualSliceAddr].ch;
	char pass = 0;

	virtualSliceMapPtr->virtualSlice[virtualSliceAddr].mh = 0;
	virtualSliceMapPtr->virtualSlice[virtualSliceAddr].ch = 0;

	if(virtualBlockMapPtr->block[copyTargetDieNo][victimBlockNo].cold == 1){
		pass = 1;
	}
	else if(tmh != 0){
		if(tch == tmh){
	...
	if(pass == 1){
			...
			{
				virtualDieMapPtr->die[dieNo].coldBlock = GetFromFbList(dieNo, GET_FREE_BLOCK_GC);
				if(virtualDieMapPtr->die[dieNo].coldBlock == BLOCK_FAIL)
					assert(!"[WARNING] There is no available block [WARNING]");
			}
			coldBlock = virtualDieMapPtr->die[dieNo].coldBlock;

			if(virtualBlockMapPtr->block[dieNo][coldBlock].currentPage == USER_PAGES_PER_BLOCK){
				SelectiveGetFromGcVictimList(dieNo, coldBlock);
				coldBlock = GetFromFbList(dieNo, GET_FREE_BLOCK_GC);

				if(coldBlock != BLOCK_FAIL)
					virtualDieMapPtr->die[dieNo].coldBlock = coldBlock;
				else
					assert(!"[WARNING] There is no available block [WARNING]");
			}
			else if(virtualBlockMapPtr->block[dieNo][coldBlock].currentPage > USER_PAGES_PER_BLOCK)
				assert(!"[WARNING] Current page management fail [WARNING]");

			virtualBlockMapPtr->block[dieNo][coldBlock].cold = 1;
		}
	else{
		dieNo = copyTargetDieNo;
		if(virtualDieMapPtr->die[dieNo].warmBlock == BLOCK_NONE)
		{
			virtualDieMapPtr->die[dieNo].warmBlock = GetFromFbList(dieNo, GET_FREE_BLOCK_GC);
			if(virtualDieMapPtr->die[dieNo].warmBlock == BLOCK_FAIL)
				assert(!"[WARNING] There is no available block [WARNING]");
		}
		coldBlock = virtualDieMapPtr->die[dieNo].warmBlock;
		...
		if(virtualBlockMapPtr->block[dieNo][coldBlock].currentPage == USER_PAGES_PER_BLOCK){
			...
			if(coldBlock != BLOCK_FAIL)
				virtualDieMapPtr->die[dieNo].warmBlock = coldBlock;
			else
				assert(!"[WARNING] There is no available block [WARNING]");
		}
		else if(virtualBlockMapPtr->block[dieNo][coldBlock].currentPage > USER_PAGES_PER_BLOCK)
		...
		virtualBlockMapPtr->block[dieNo][coldBlock].cold = 2;
	}

	virtualSliceMapPtr->virtualSlice[virtualSliceAddr].mh = tmh;
	virtualSliceMapPtr->virtualSlice[virtualSliceAddr].ch = tch;


	virtualSliceAddr = Vorg2VsaTranslation(dieNo, coldBlock, virtualBlockMapPtr->block[dieNo][coldBlock].currentPage);
	virtualBlockMapPtr->block[dieNo][coldBlock].currentPage++;
...
void InvalidateOldVsa(unsigned int logicalSliceAddr)
{
	...
	if(virtualSliceAddr != VSA_NONE)
	{
		...
		// unlink
		//SelectiveGetFromGcVictimList(dieNo, blockNo); // 2R ������ ����
		...
		//PutToGcVictimList(dieNo, blockNo, virtualBlockMapPtr->block[dieNo][blockNo].invalidSliceCnt); // // 2R ������ ����
	}
...
void EraseBlock(unsigned int dieNo, unsigned int blockNo)
{
	...
	virtualBlockMapPtr->block[dieNo][blockNo].cold = 0;
